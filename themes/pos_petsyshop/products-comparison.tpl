...
<h1 class="page-title">{l s='Product Comparison'}</h1>
		...
		{foreach from=$products item=product name=for_products}
			...
			<td width="{$width}%" class="ajax_block_product comparison_infos">
				<a href="{$product->getLink()}" title="{$product->name|escape:html:'UTF-8'}" class="product_image" >
					<img src="{$link->getImageLink($product->link_rewrite, $product->id_image, 'pos_product')|escape:'html'}" alt="{$product->name|escape:html:'UTF-8'}" width="{$homeSize.width}" height="{$homeSize.height}" />
				</a>

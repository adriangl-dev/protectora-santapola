...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-right-column" class="span5">
		...
		<div class="image_view_block">
			...
			{if $have_image}
				<span id="view_full_size">
					<meta property="og:image" content="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'pos_large')|escape:'html'}"/>
					<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'pos_large')|escape:'html'}"{if $jqZoomEnabled && $have_image} class="jqzoom"{/if} title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" id="bigpic"/>
		...
		<!-- thumbnails -->
			...
			{if isset($images) && count($images) > 4}<span class="view_scroll_spacer"><a id="view_scroll_left" class="hidden thumbs" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}">{l s='Previous'}</a></span>{/if}
			<div id="thumbs_list">
				<ul id="thumbs_list_frame">
					{if isset($images)}
						...
						<li id="thumbnail_{$image.id_image}">
							<a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'pos_thickbox')|escape:'html'}" rel="other-views" class="thickbox{if $smarty.foreach.thumbnails.first} shown{/if}" title="{$image.legend|htmlspecialchars}">
								<img id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'pos_medium')|escape:'html'}" alt="{$image.legend|htmlspecialchars}" height="{$mediumSize.height}" width="{$mediumSize.width}" />
							</a>
						</li>
						{/foreach}
					{/if}
			...
			{if isset($images) && count($images) > 4}<a class="thumbs" id="view_scroll_right" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}">{l s='Next'}</a>{/if}
			</div>
	</div>
		{/if}
	...
	<div id="pb-left-column" class="span7">
		...
		<div class="content_prices clearfix">
			...
			{if $product->id_category_default!=16 && $product->id_category_default!=21 && $product->id_category_default!=23}
			<p class="buttons_bottom_block">
				<a href="https://protectorasantapola.org/index.php?id_cms=12&controller=cms" id="wishlist_button" title="Quiero adoptarlo" rel="nofollow">Quiero adoptar a {$product->name|escape:'htmlall':'UTF-8'}</a>
			</p>
		
			<p class="buttons_bottom_block">
				<a href="https://protectorasantapola.org/index.php?id_cms=12&controller=cms" id="wishlist_button" title="Tengo una casa de acogida para él" rel="nofollow">Tengo una casa de acogida para {$product->name|escape:'htmlall':'UTF-8'}</a>
			</p>
			{/if}
						...
						{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<li class="ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
								...
								<div class="product_desc">
									<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{$accessory.legend|escape:'htmlall':'UTF-8'}" class="product_image"><img src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'pos_img_medium')|escape:'html'}" alt="{$accessory.legend|escape:'htmlall':'UTF-8'}" width="{$mediumSize.width}" height="{$mediumSize.height}" /></a>
...
{/if}
{/if}
{if isset($HOOK_PRODUCT_FOOTER) && $HOOK_PRODUCT_FOOTER && $product->id_category_default!=16 && $product->id_category_default!=21 && $product->id_category_default!=23}{$HOOK_PRODUCT_FOOTER} {/if}

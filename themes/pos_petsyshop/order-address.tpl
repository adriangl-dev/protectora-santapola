...
{if !$opc}<h1 class="page-title">{l s='Addresses'}</h1>{else}<h2 class="page-title"><span>1</span> {l s='Addresses'}</h2>{/if}
...
{/if}
	<div class="addresses clearfix">
		...
		<div class="clearfix row-fluid">
			<ul class="address item span6" id="address_delivery" {if $cart->isVirtualCart()}style="display:none;"{/if}>
			</ul>
			<ul class="address span6 alternate_item {if $cart->isVirtualCart()}full_width{/if}" id="address_invoice">
			</ul>
		</div>

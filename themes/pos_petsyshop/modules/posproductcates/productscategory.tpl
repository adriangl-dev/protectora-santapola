                            ...
                            {foreach from=$categoryProducts item='categoryProduct' name=categoryProduct}
                                   <li >
                                        <div class="posproductcategory-item-inner">
                                            <a href="{$link->getProductLink($categoryProduct.id_product, $categoryProduct.link_rewrite, $categoryProduct.category, $categoryProduct.ean13)}" class="lnk_img" title="{$categoryProduct.name|htmlspecialchars}"><img src="{$link->getImageLink($categoryProduct.link_rewrite, $categoryProduct.id_image, 'pos_product')|escape:'html'}" alt="{$categoryProduct.name|htmlspecialchars}" /></a>
                                            ...
                                            {if $slideOptions.show_des ==1 }
                                                <div class="product_desc"><a href="{$categoryProduct.link|escape:'html'}" title="{l s='More' mod='posproductcates'}">{$categoryProduct.description_short|strip_tags|truncate:65:'...'}</a></div>
                                            {/if}

...
class Virgyfeatureproduct extends Module {

    private $_html = '';
    private $_postErrors = array();

    function __construct() {
        $this->name = 'virgyfeatureproduct';
        ...
        $this->displayName = $this->l('Perros en adopción como slider en la home.');
        $this->description = $this->l('Muestra los últimos perros en adopción como slider en la home.');
    }

    function install() {
        $this->_clearCache('virgyfeatureproduct.tpl');
    ...
    public function hookHeader($params) {
        $this->context->controller->addCSS(($this->_path) . 'css/virgyfeatureproduct.css', 'all');
...
//                $this->context->controller->addJS($this->_path.'js/jquery.elastislide.js');
    ...
    public function getSlideshowHtml() {
        ...
        if (!$this->isCached('virgyfeatureproduct.tpl', $this->getCacheId('virgyfeatureproduct'))) {
            $slideOptions = array(
			...
			$category = new Category(13, (int) Context::getContext()->language->id);
        ...
        return $this->display(__FILE__, 'virgyfeatureproduct.tpl', $this->getCacheId('virgyfeatureproduct'));
    }

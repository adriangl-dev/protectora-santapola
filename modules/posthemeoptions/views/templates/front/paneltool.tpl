  ...
  <form action="{$content_dir}index.php" method="get">
    <div style="min-height: 300px;   left: 0px;" class="pn-content inactive" id="toolspanelcontent">
      ...
      <div id="template_theme"> 
        ...
        <select name="skin">
        {foreach from=$POS_THEMEINFO.skins item=skin}
          <option value="{$skin}" {if $POS_SKIN_DEFAULT==$skin}selected="selected"{/if}>{$skin}</option>
        {/foreach} 
        </select>
       
      </div>
     
      {if isset($POS_THEMEINFO.patterns)}
      <div id="pnpartterns">
        <h5>Pattern</h5>
		<input type="hidden" value="" name="bgpattern" id="bgpattern"/>
        {foreach from=$POS_THEMEINFO.patterns item=p}
        	<a style="background:url('{$content_dir}modules/{$POS_MODULENAME}/img/patterns/{$p}')" onclick="return false;" href="#" title="{$p}" id="{$p}" class="{if $POS_PATTERN == $p}active{/if}">
                </a>
        {/foreach}
      ...
      <div class="clearfix" id="bottombox">
        ...
        <a href="{$content_dir}index.php?posaction=reset" class="button btn">Reset</a></div>
      <div class="clearfix"></div>
    </div>
  </form>
</div>

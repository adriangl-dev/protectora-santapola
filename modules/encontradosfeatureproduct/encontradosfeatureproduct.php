...
class Encontradosfeatureproduct extends Module {

    private $_html = '';
    private $_postErrors = array();

    function __construct() {
        $this->name = 'encontradosfeatureproduct';
        ...
        $this->displayName = $this->l('Encontrados como slider en la home.');
        $this->description = $this->l('Muestra los encontrados como slider en la home.');
    }

    function install() {
        $this->_clearCache('encontradosfeatureproduct.tpl');
        ...
        Configuration::updateValue($this->name . '_width_item', 250);
    ...
    public function hookHeader($params) {
        $this->context->controller->addCSS(($this->_path) . 'css/encontradosfeatureproduct.css', 'all');
...
//                $this->context->controller->addJS($this->_path.'js/jquery.elastislide.js');
    ...
    public function getSlideshowHtml() {
        ...
        if (!$this->isCached('encontradosfeatureproduct.tpl', $this->getCacheId('encontradosfeatureproduct'))) {
            $slideOptions = array(
                'auto_slide' => Configuration::get($this->name . '_auto_slide'),
                'title_slide' => Configuration::get($this->name . '_title_slide'),
			...
			$category = new Category(23, (int) Context::getContext()->language->id);
        ...
        return $this->display(__FILE__, 'encontradosfeatureproduct.tpl', $this->getCacheId('encontradosfeatureproduct'));
    }
